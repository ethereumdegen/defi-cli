...
func (s *runtimeState) newYieldVerbExecutionCommand(verb actionbuilder.YieldVerb, short string) *cobra.Command {
	...
	runCmd := &cobra.Command{
		Use:   "run",
		Short: "Plan and execute a yield action",
		RunE: func(cmd *cobra.Command, _ []string) error {
	...
	runCmd.Flags().StringVar(&run.provider, "provider", "", "Yield provider (aave|morpho)")
	...
	runCmd.Flags().StringVar(&run.vaultAddress, "vault-address", "", "Morpho vault address (required for --provider morpho)")
